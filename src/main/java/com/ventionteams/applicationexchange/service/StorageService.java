...
public class StorageService {
    ...
    private String getURL(String keyName) {
        try {
            GetUrlRequest request = GetUrlRequest.builder()
                    ...
                    .key(keyName)
                    .build();
            log.info("Upload file with name: {}", keyName);
    ...
    public void delete(String name) {
        s3Client.deleteObject(DeleteObjectRequest.builder().bucket(configProperties.getBucketName()).key(name).build());
        log.info("Delete file with name: {}", name);
    }
}
